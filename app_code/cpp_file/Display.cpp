...
namespace
{
    ...
    // file descriptor for /dev/gpiomem
    int gpio_fd = -1;

    // size of mapped GPIO memory block
    constexpr size_t GPIO_BLOCK_SIZE = 4096;

    // GPIO register offsets (32-bit word addressing)
    ...
    static inline void write_command(uint32_t cmd) {
        gpio_base[GPCLR0] = 0x500000;
        write_bus(cmd);
        wr_strobe();
        gpio_base[GPSET0] = 0x100000;
    }
...
/*
 * Summary:
 * Map GPIO memory into user space.
 *
 * Parameters:
 * - No parameters.
 *
...
/*
 * Summary:
 * Unmap GPIO memory and close file descriptor.
 *
 * Parameters:
 * - No parameters.
 *
...
/*
 * Summary:
 * Initialize GPIO configuration for display.
 *
 * Parameters:
 * - No parameters.
 *
...
/*
 * Summary:
 * Send command followed by data buffer to display.
 *
 * Parameters:
 * - cmd (uint32_t): command to send
 * - data (uint32_t*): pointer to data buffer
 * - length (int): number of data elements
 *
 * Returns:
 * - No return value.
 */
...
/*
 * Summary:
 * Initialize display controller and perform startup sequence.
 *
 * Parameters:
 * - No parameters.
 *
...
/*
 * Summary:
 * Fill entire display with single color.
 *
 * Parameters:
 * - color (uint32_t): color value to fill
 *
 * Returns:
 * - No return value.
 */
...
/*
 * Summary:
 * Fill screen with two colors split into two regions.
 *
 * Parameters:
...
/*
 * Summary:
 * Display an OpenCV image on screen.
 *
 * Parameters:
 * - image (cv::Mat): input image
 * - part (int): screen region selector
 *
 * Returns:
 * - No return value.
 */
